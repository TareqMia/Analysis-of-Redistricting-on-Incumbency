...
const District = ({ currentState, currentDistrict }) => {
    ...
    return (<div className='district'>
        <h1><strong>District Details</strong></h1>
        <strong>District:</strong> {dist}<br/>
        <strong>Winner:</strong> {candid.Candidate}<br/>
        <strong>2022 Congressional Result:</strong> {candid.Party}<br/>
                ...
                if (val.District === dist) {
                    return(
                        <tr key={key}>
                            ...
                            <td>{(val["Black or African American"] / (total * 1.0) * 100).toFixed(2)}</td>
                            <td>{(val["American Indian and Alaska Native"] / (total * 1.0) * 100).toFixed(2)}</td>
                            <td>{(val.Asian / (total * 1.0) * 100).toFixed(2)}</td>
                            <td>{(val["Native Hawaiian and Other Pacific Islander"] / (total * 1.0) * 100).toFixed(2)}</td>
